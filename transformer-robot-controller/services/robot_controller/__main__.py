...
from services.mir_controller.mir_interface import MiRController
...
def convert_to_axis_motion_command(command: Union[HTTPAxisPositioningCommand, HTTPAxisTorqueCommand]) -> AxisPositioningCommand:
    if type(command) == HTTPAxisPositioningCommand:
        converted_command = AxisPositioningCommand(target_position=None)
    elif type(command) == HTTPAxisTorqueCommand:
        converted_command = AxisTorqueCommand(target_torque=None, maximum_position=None)
    else:
...
def convert_to_robot_motion_command(command: HTTPRobotMotionCommand) -> RobotMotionCommand:
...
def handle_motion_command(command: HTTPRobotPositioningCommand) -> HTTPResponse:
    #if type(command) == HTTPRobotPositioningCommand
    ...
    response = motion_controller.enqueue_motion_command(command)
...
async def position_axis(axis_label: str, command: HTTPAxisPositioningCommand):
    ...
    robot_command = HTTPRobotMotionCommand()
...
@app.post("/axes/{axis_label}/motion/torque")
async def torque_axis(axis_label: str, command: HTTPAxisTorqueCommand):
    if axis_label not in motion_controller.axes_installed:
        return HTTPResponse(error_string="Axis {} is not installed in machine".format(axis_label))

    if command.target_torque is None:
        return HTTPResponse(error_string="Must specify a non-null value for \"target_torque\", not {}".format(str(command.target_torque)))

    if command.maximum_position is None:
        return HTTPResponse(error_string="Must specify a non-null value for \"maximum_position\", not {}".format(str(command.maximum_position)))
...
def setup_logger():
    if not os.path.exists(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY)):
        os.mkdir(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY))

    for file in [f for f in os.listdir(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY)) if os.path.isfile(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY, f))]:
        os.remove(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY, file))
...
if __name__ == "__main__":
    ...
    logger.add(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY, "3r_motion_log_{time}.csv"), format="{message}", rotation="5 MB",
               retention="0.5h", enqueue=True, filter=motion_only, level='TRACE')
    logger.add(os.path.join(ROOT_PATH, LOG_FILE_DIRECTORY, '3r_error_log_{time}.log'), format='{time} {message}', rotation="5 MB", level='ERROR')
    ...
    try:
        ...
        #mir200 = MiRController(configuration_file_name='mir200_config.yaml')
        #mir200.start()
