...
class Table:
    ...
    def set_table_clamp(self, clamped: bool) -> bool:
        if self.control_type == 'festo':
            ...
            if self.compax3_clamp_io_point is not None:
                self.parent_machine.cell_controller.axes[self.compax3_clamp_io_point['bound_axis_label']].set_IO_point(data_link=self.parent_machine.cell_controller.port,
                                                        io_point_address=self.compax3_unclamp_io_point['point_address'],
                                                        io_point_value=not clamped)

            if self.compax3_unclamp_io_point is not None:
                self.parent_machine.cell_controller.axes[self.compax3_unclamp_io_point['bound_axis_label']].set_IO_point(data_link=self.parent_machine.cell_controller.port,
                                                        io_point_address=self.compax3_unclamp_io_point['point_address'],
                                                        io_point_value=not clamped)
    ...
    def check_table_unclamped(self) -> bool:
        ...
        return unclamped
            # self.axes[self.compax3_clamp_io_point_address[0]].set_IO_point(data_link=self.port,
            ...
            #     self.axes[unclamp_bound_axis_label].set_IO_point(data_link=self.port,
            ...
            #     self.axes[clamp_bound_axis_label].set_IO_point(data_link=self.port,
