...
from services.robot_controller.utils import Compax3DataLink
...
class Compax3M:
    ...
    default_torque: float = 0
    default_torque_ramp_rate: float = 0
    ...
    maximum_torque: float = 0
    maximum_torque_ramp_rate: float = 0
    ...
    def __post_init__(self):
        ...
        for status_bit in self.status_bit_definitions:
            self.drive_status_bits[status_bit] = False
    ...
    def write_message_sequence(self, data_link: Compax3DataLink, message_sequence: List[Dict], value_sequence: List = None) -> str:
        if self.comm_format == 'ascii':
            ...
            data_link.flush_input()
            data_link.write(message_payload)
            ...
            # if self.communication_configuration['use_serial_server']:
            #     socket.write(message_payload)
            # else:
    ...
    def receive_payload(self, data_link: Compax3DataLink, message_sequence: List[Dict], processing_callback: Any, callback_args: Dict = {}) -> MessageProcessed:#, callback_args: Dict):
        if self.comm_format == 'ascii':
            ...
            while not processing_status.received and attempts <= self.communication_configuration['receive_attempts']:
                buffer += data_link.read(self.communication_configuration['receive_chunk_size'])
    ...
    def guaranteed_write_read(self, data_link: Compax3DataLink, message_sequence: List[Dict], process_callback: Any, value_sequence: List = None, callback_args: Dict = {}) -> MessageProcessed:
        update_attempts = 0
        while update_attempts <= self.maximum_update_attempts:
            if self.comm_format == 'ascii':
                try:
                    message_payload = self.write_message_sequence(data_link=data_link, message_sequence=message_sequence, value_sequence=value_sequence)
                    processing_status = self.receive_payload(data_link,
                                                             message_sequence,
                                                             process_callback,
                                                             callback_args)
                ...
                try:
                    self.write_message_sequence(data_link=data_link, message_sequence=message_sequence, value_sequence=value_sequence)
                    processing_status = self.receive_payload(data_link=data_link, message_sequence=message_sequence, processing_callback=None)
    ...
    def update_axis(self, data_link: Compax3DataLink, multiline: bool = False):
        if self.comm_format == 'ascii':
            try:
                if not multiline:
                    for message in update_message_sequence:
                        data_link.write(message['message_type'](axis_address=self.axis_address if self.communication_configuration['communication_type'] == 'rs485' else None).Serialize())
                        setattr(self, message['attribute_name'], message['message_type'](received_data=data_link.readline()).get_value)
                else:
                    # First update the current axis status
                    self.guaranteed_write_read(data_link=data_link,
            ...
            try:
                self.guaranteed_write_read(data_link=data_link, message_sequence=update_message_sequence, process_callback=None)
                self.update_time = time.time()
    ...
    def check_command_default_dynamics(self, command: AxisPositioningCommand) -> AxisPositioningCommand:
    ...
    def update_axis_commands(self, data_link: Compax3DataLink, command: Union[AxisPositioningCommand, AxisTorqueCommand], multiline: bool = False):
        if self.comm_format == 'ascii':
            if not multiline:
                raise NotImplementedError
            else:
                ack = self.guaranteed_write_read(data_link=data_link,
                                                 message_sequence=absolute_position_command_message_sequence,
                                                 process_callback=self.process_received_multiline_set_acknowledgements,
                                                 value_sequence=[getattr(command, message['command_attribute_name']) for message in absolute_position_command_message_sequence]).correct

                if ack:

                    # read back axis commands to ensure they were written correctly
                    set_values_matched = self.guaranteed_write_read(data_link=data_link, message_sequence=absolute_position_command_message_sequence,
            ...
            if not multiline:
                ...
                success = True
                if type(command) == AxisPositioningCommand:
                    message_sequence = absolute_position_command_message_sequence
                elif type(command) == AxisTorqueCommand:
                    message_sequence = torque_command_message_sequence
                
                for message in message_sequence:
                    try:
                        success &= self.guaranteed_write_read(data_link=data_link,
                        ...
                        value_check = self.guaranteed_write_read(data_link=data_link,
                                                              message_sequence=[message],
                                                              process_callback=None)
    ...
    def enable_axis(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'ascii':
            success = self.guaranteed_write_read(data_link=data_link,
        ...
        if success:
            logger.info(f'Sent {self.axis_label} axis enable command')
        else:
            logger.error(f'Failed to send {self.axis_label} axis enable command')
        return success

    def execute_move(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'ascii':
            success = self.guaranteed_write_read(data_link=data_link,
    ...
    def set_motion_mode(self, data_link: Compax3DataLink, command: Union[AxisPositioningCommand, AxisTorqueCommand]) -> bool:
        if self.comm_format == 'binary':
            if type(command) == AxisPositioningCommand:
                message_sequence = absolute_position_mode_message_sequence
            elif type(command) == AxisTorqueCommand:
                message_sequence = torque_mode_message_sequence
    ...
    def home_axis(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'binary':
            success = True
    ...
    def reset_axis(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'binary':
    ...
    def disable_axis(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'binary':
    ...
    def stop_axis(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'binary':
    ...
    def set_IO_point(self, data_link: Compax3DataLink, io_point_address: int, io_point_value: bool) -> bool:
    ...
    def acknowledge_command_error(self, data_link: Compax3DataLink) -> bool:
        if self.comm_format == 'binary':
