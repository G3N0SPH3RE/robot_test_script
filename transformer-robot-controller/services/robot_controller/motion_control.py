...
from .messages import AxisPositioningCommand, AxisCommand, AxisTorqueCommand, RobotMotionCommand, RobotCommand
from .hardware_interface import Compax3M
from .utils import MachineConfigurationException, MotionControllerException, Compax3DataLink, drain_queue
...
class MotionController(Machine, Thread):
    def __init__(self, configuration_file_name: str, calibration_table_name: str, motion_data_stream_address: Tuple[str, int]):
        ...
        self.machine_command_retry_queue: Queue[RobotCommand] = Queue()
        self.sequence_queue: Queue[MachineSequence] = Queue()
        self.command_retries: int = 0
        ...
        self.compax3_data_link: Compax3DataLink = None
        ...
        self.initialize_drive_communication()
    ...
    def initialize_drive_communication(self):
        self.compax3_data_link = Compax3DataLink(self.communication_configuration)
    ...
    def set_pallet_clamp(self, clamped: bool) -> bool:
        if "gripper_open" in self.io_point_configuration['output'].keys() and "gripper_close" in self.io_point_configuration['output'].keys() and not self.simulate_io:
            ...
            self.axes[bound_axis_label].set_IO_point(data_link=self.compax3_data_link,
    ...
    def set_table_clamp(self, clamped: bool) -> bool:
        ...
        #     self.axes[bound_axis_label].set_IO_point(data_link=self.port,
        #                                              io_point_address=point_address,
        #                                              io_point_value=not clamped)
    ...
    def set_motion_position_target(motion_controller: object):
        for axis_function, waypoint_label in motion_controller.current_sequence_waypoints[motion_controller.sequence_state_machine.state].items():
            if axis_function in motion_controller.axis_function_map.keys() and waypoint_label is not None:# and not motion_controller.simulate_axes:
                ...
                # target_axis.update_axis_commands(data_link=motion_controller.port, command=AxisMotionCommand(
                #     target_position=getattr(motion_controller.current_machine_sequence, waypoint_label)))
                ...
                target_axis.update_axis_commands(data_link=motion_controller.compax3_data_link, command=AxisPositioningCommand(
                    target_position=target_position))
                #target_axis.position_absolute(data_link=motion_controller.compax3_data_link)
                target_axis.set_motion_mode(data_link=motion_controller.compax3_data_link, command=AxisPositioningCommand())
                target_axis.execute_move(data_link=motion_controller.compax3_data_link)
    ...
    #         traverse_axis.update_axis_commands(data_link=motion_controller.port, command=AxisMotionCommand(
    #             target_position=motion_controller.current_machine_sequence.traverse_end))
    #         traverse_axis.position_absolute(data_link=motion_controller.port)
    #         traverse_axis.execute_move(data_link=motion_controller.port)
    ...
    #         swing_axis.update_axis_commands(data_link=motion_controller.port, command=AxisMotionCommand(
    #             target_position=motion_controller.current_machine_sequence.swing_end))
    #         swing_axis.position_absolute(data_link=motion_controller.port)
    #         swing_axis.execute_move(data_link=motion_controller.port)
    ...
    #         vertical_axis.update_axis_commands(data_link=motion_controller.port, command=AxisMotionCommand(
    ...
    #         vertical_axis.position_absolute(data_link=motion_controller.port)
    #         vertical_axis.execute_move(data_link=motion_controller.port)
    ...
    #         reach_axis.update_axis_commands(data_link=motion_controller.port, command=AxisMotionCommand(
    #             target_position=motion_controller.current_machine_sequence.reach_end))
    #         reach_axis.position_absolute(data_link=motion_controller.port)
    #         reach_axis.execute_move(data_link=motion_controller.port)
        ...
        if not all([axis.axis_running for axis in self.axes.values()]) and not self.motion_queue.empty():
            ...
            for axis_field in fields(motion_command):
                ...
                if axis_command is not None:
                    # if type(axis_command) == AxisPositioningCommand:
                    #     message_sequence = absol
                    motion_update_complete &= self.axes[axis_label].update_axis_commands(data_link=self.compax3_data_link,
                                                                                         command=axis_command,
                                                                                         multiline=False)
                    motion_update_complete &= self.axes[axis_label].set_motion_mode(data_link=self.compax3_data_link, command=axis_command)
            ...
            if (self.machine_setup_configuration['plc_installed']):
                ...
                for axis_field in fields(motion_command):
                    ...
                    if axis_command is not None:
                        self.axes[axis_field.name].execute_move(self.compax3_data_link)
    ...
    def enqueue_motion_command(self, command: RobotMotionCommand) -> MotionControllerCommandResponse:
    ...
    def check_limits(self, command: Union[MachineSequence, RobotMotionCommand]) -> MotionControllerCommandResponse:
        for axis_field in fields(command):
            if type(command) == RobotMotionCommand:
                ...
                if type(getattr(command, axis_label)) == AxisTorqueCommand:
                    target_torque = getattr(command, axis_label).target_torque
                    maximum_position = getattr(command, axis_label).maximum_position
                    maximum_velocity = getattr(command, axis_label).maximum_velocity
                    maximum_acceleration = getattr(command, axis_label).maximum_acceleration
                    maximum_deceleration = getattr(command, axis_label).maximum_deceleration
                    torque_ramp_rate = getattr(command, axis_label).torque_ramp_rate
                elif type(getattr(command, axis_label)) == AxisPositioningCommand:
            ...
            if type(getattr(command, axis_label)) == AxisPositioningCommand:
                ...
                if target_deceleration_jerk is not None and target_deceleration_jerk > self.axes[
                    ...
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past deceleration jerk limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_jerk)))
            elif type(getattr(command, axis_label)) == AxisTorqueCommand:
                if target_torque is not None and target_torque > self.axes[axis_label].maximum_torque:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past torque limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_torque)))
                if maximum_position < self.axes[axis_label].negative_limit:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past negative limit of {}".format(axis_label, str(
                            self.axes[axis_label].negative_limit)))
                if maximum_position > self.axes[axis_label].positive_limit:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past positive limit of {}".format(axis_label, str(
                            self.axes[axis_label].positive_limit)))
                if maximum_velocity is not None and maximum_velocity > self.axes[axis_label].maximum_velocity:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past velocity limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_velocity)))
                # if maximum_velocity is not None and maximum_velocity > self.axes[axis_label].maximum_velocity:
                ...
                #         success=False, response_string="Axis {} command past velocity limit of {}".format(axis_label, str(
                #             self.axes[axis_label].maximum_velocity)))
                if maximum_acceleration is not None and maximum_acceleration > self.axes[axis_label].maximum_acceleration:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past acceleration limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_acceleration)))
                if maximum_deceleration is not None and maximum_deceleration > self.axes[axis_label].maximum_acceleration:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past deceleration limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_acceleration)))
                if torque_ramp_rate is not None and torque_ramp_rate > self.axes[axis_label].maximum_torque_ramp_rate:
                    return MotionControllerCommandResponse(
                        success=False, response_string="Axis {} command past torque ramp rate limit of {}".format(axis_label, str(
                            self.axes[axis_label].maximum_torque_ramp_rate)))
    ...
    def process_machine_command(self, command: RobotCommand):
        ...
        if command.command_type == ExternalCommandType.SET_IO_POINT:
            if command.point_address is not None:
                ...
                self.axes[target_axis_label].set_IO_point(data_link=self.compax3_data_link,
                ...
                self.axes[bound_axis_address].set_IO_point(data_link=self.compax3_data_link,
                                                           io_point_address=point_address,
                                                           io_point_value=command.point_state)
            ...
            for command_field in fields(command):
                ...
                if command.command_type == ExternalCommandType.ENABLE:
                    if self.simulate_axes:
                        self.axes[command_field.name].enable_axis(self.compax3_data_link)
                    elif not self.axes[command_field.name].drive_status_bits['power_enabled']:
                        self.machine_command_retry_queue.put(command)
                        self.axes[command_field.name].enable_axis(self.compax3_data_link)
                    else:
                        logger.info(f'Skipping command {command.command_type} since axis {command_field.name} is already enabled')
                        drain_queue(self.machine_command_retry_queue)
                    ...
                    if command_field.name in self.axes.keys():
                        self.axes[command_field.name].home_axis(self.compax3_data_link)
                        self.axes[command_field.name].execute_move(self.compax3_data_link)
                    else:
                        print("UNKNOWN AXIS REQUESTED FOR HOMING")
                elif command.command_type == ExternalCommandType.RESET:
                    self.axes[command_field.name].reset_axis(data_link=self.compax3_data_link)
                elif command.command_type == ExternalCommandType.DISABLE:
                    self.axes[command_field.name].disable_axis(data_link=self.compax3_data_link)
                    #raise NotImplementedError
                elif command.command_type == ExternalCommandType.STOP:
                    #self.axes[command.axis_label].stop_axis(data_link=self.port)
                    self.axes[command_field.name].stop_axis(data_link=self.compax3_data_link)
    ...
    def run(self):
        while self.run_flag:
            for axis_label, axis in self.axes.items():
                with self.data_lock:
                    axis.update_axis(self.compax3_data_link, multiline=True)

                time.sleep(self.communication_configuration['update_delay'])
            self.log_axis_motion()

            if not self.machine_command_retry_queue.empty():
                retry_command = self.machine_command_retry_queue.get()
                self.process_machine_command(retry_command)
                self.command_retries += 1
                if self.command_retries >= self.communication_configuration['maximum_command_retries']:
                    logger.error(f"Failed to complete command {retry_command}")
                    drain_queue(self.machine_command_retry_queue)
            else:
